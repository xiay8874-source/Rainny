...
function globalConfigFile() {
  const candidates = ["rainny.jsonc", "rainny.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "rainny.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "rainny.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".rainny") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["rainny.json", "rainny.jsonc"]) {
          ...
          // Auto-discovered plugins under `.rainny/plugin(s)` are already local files, so ConfigPlugin.load

...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in rainny.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .rainny/ subdirectory too)
  const candidates = [path.join(baseDir, "rainny.json"), path.join(baseDir, "rainny.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".rainny", "rainny.json"), path.join(baseDir, ".rainny", "rainny.jsonc"))
  }
  ...
  // Default to rainny.json if none exist
  return candidates[0]
}

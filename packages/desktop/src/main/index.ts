...
import { APP_IDS, APP_NAMES, PRODUCT } from "../branding"
...
const main = Effect.gen(function* () {
  ...
  const onboardingTestRoot = ((): string | undefined => {
    ...
    const root = join(tmpdir(), `rainny-onboarding-${randomUUID()}`)
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter(
      (arg: string) => arg.startsWith(`${PRODUCT.protocol}://`) || arg.startsWith("opencode://"),
    )
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient(PRODUCT.protocol)
  registerRendererProtocol()
  setDockIcon()

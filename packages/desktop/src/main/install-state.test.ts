...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier OpenCode launch", () => {
    expect(hasExistingAppState([file("rainny.settings")])).toBe(true)
    expect(hasExistingAppState([file("rainny.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("rainny")])).toBe(true)
  })
})

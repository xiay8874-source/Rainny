...
await $`cp ./bin/lildax.cjs ./dist/${pkg.name}/bin/rainny`
await Bun.file(`./dist/${pkg.name}/package.json`).write(
  JSON.stringify(
    {
      name: pkg.name,
      bin: { rainny: "./bin/rainny" },
      version,
      license: pkg.license,
      repository: { type: "git", url: "git+https://github.com/xiay8874-source/Rainny.git" },
...
await Promise.all(
  Object.entries(binaries).map(([name, version]) =>
    publish(`./dist/${name.replace("@rainny-ai/", "")}`, name, version),
  ),
)
